...
cdef extern from "RVOSimulator.h" namespace "RVO":
    cdef cppclass RVOSimulator:
        ...
        void setGoalProg(size_t agentNo, float avgGoalProg,int actionNum)
        float getAgentGoalProg(size_t agentNo, int actionNum)
...
cdef class PyRVOSimulator:
    ...
    def getAgentPosition(self, size_t agent_no):
        ...
        return pos.x(), pos.y()
    def getAgentGoal(self, size_t agent_no):
        cdef Vector2 goal = self.thisptr.getAgentGoal(agent_no)
    ...
    def setAgentGoal(self, size_t agent_no, tuple goal):
        cdef Vector2 c_goal = Vector2(goal[0], goal[1])
        self.thisptr.setAgentGoal(agent_no, c_goal)
    def setAgentGoalPrefVelocity(self, size_t agent_no, tuple goalprefVelocity):
        cdef Vector2 c_goalprefVelocity = Vector2(goalprefVelocity[0], goalprefVelocity[1])
        self.thisptr.setAgentGoalPrefVelocity(agent_no, c_goalprefVelocity)
